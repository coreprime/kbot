...
export class GameEngine {
  ...
  tick(dtMs, { skipRuntime = false, skipMovement = false, skipSync = false } = {}) {
    // Every caller is now a single per-frame driver (the tab-owned tick
    // loop in /ui/sandbox/tab.js + /ui/unit-editor/tab.js, plus the Step
    // button).  No more renderer.onAfterFrame self-ticking — the old
    // wall-clock dedup that guarded against that pattern has been removed
    // so engine.tick is deterministic: every call advances the sim.
