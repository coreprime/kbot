...
func runRoundtrip(root string, detailed bool) error {
	...
	fmt.Fprintf(os.Stderr,  "  | %-45s|\n", fmt.Sprintf("Decompile -> Compile:     %-11s %s", recompFrac, passFail(recompPass == totalFiles)))
	fmt.Fprintf(os.Stderr,  "  | %-45s|\n", fmt.Sprintf("Disassemble -> Assemble:  %-11s %s", reasmFrac, passFail(reasmPass == totalFiles)))
	if allPass {
		fmt.Fprintln(os.Stderr, "  |                                              |")
...
func testOneFile(path string, detailed bool) roundtripResult {
	...
	if err != nil {
		...
		if err != nil {
			...
			r.RecompHash = md5hex(recompData)
