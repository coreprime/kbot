...
// newTAFCommand builds the `kbot taf` command tree for TA: Kingdoms truecolor
// animations.
func newTAFCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "taf",
...
func loadTAF(arg, vfsRoot string, quiet bool) (*tsf.TAF, *VFSInputHit, func(), error) {
	...
	hit, err := resolveVFSInput(arg, vfs, ".taf", []string{"anims/", "textures/"})
	if err != nil {
		cleanup()
		return nil, nil, func() {}, err
	}
...
// resolveTSFInput resolves a .tsf argument through the active VFS.
